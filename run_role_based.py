...
This script executes the role-based prompt technique across all 100 test cases,
running each case 2 times to measure consistency. Results are saved to:
- results/role_based_results.csv (raw data)
- results/role_based_stats.json (aggregated statistics)
"""
...
def main() -> None:
    """Run the role-based prompt experiment and generate statistics."""
    print("=" * 60)
    print("Stage 3: Role-Based Prompt Experiment (Ollama)")
    print("=" * 60)
    ...
    prompt_generator = RoleBasedPromptGenerator()
    ...
    # Run role-based prompt experiment
    print(f"\n[3/5] Running role-based prompt experiment ({total_calls} API calls)...")
    ...
    # Save stats JSON
    stats_path = results_dir / "role_based_stats.json"
    with open(stats_path, "w") as f:
        json.dump(stats, f, indent=2)
    print(f"  Saved: {stats_path}")

    # Also copy raw results to expected location
    raw_results_path = results_dir / "role_based_results.csv"
    try:
        ...
        alt_path = results_dir / f"role_based_results_{timestamp}.csv"
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("ROLE-BASED PROMPT EXPERIMENT RESULTS")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("Role-Based Prompt Experiment Complete!")
    print("=" * 60)
