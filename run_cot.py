...
This script executes the chain-of-thought prompt technique across all 100 test cases,
running each case 2 times to measure consistency. Results are saved to:
- results/cot_results.csv (raw data)
- results/cot_stats.json (aggregated statistics)
"""
...
def main() -> None:
    """Run the chain-of-thought prompt experiment and generate statistics."""
    print("=" * 60)
    print("Stage 3: Chain-of-Thought Prompt Experiment (Ollama)")
    print("=" * 60)
    ...
    prompt_generator = ChainOfThoughtPromptGenerator()
    ...
    # Estimate time for Ollama (CoT may take longer due to verbose output)
    est_minutes = (total_calls * 7) / 60
    print(f"  Estimated time: ~{est_minutes:.0f}-{est_minutes*2:.0f} minutes")

    # Run chain-of-thought prompt experiment
    print(f"\n[3/5] Running chain-of-thought prompt experiment ({total_calls} API calls)...")
    ...
    # Save stats JSON
    stats_path = results_dir / "cot_stats.json"
    with open(stats_path, "w") as f:
        json.dump(stats, f, indent=2)
    print(f"  Saved: {stats_path}")

    # Also copy raw results to expected location
    raw_results_path = results_dir / "cot_results.csv"
    try:
        ...
        alt_path = results_dir / f"cot_results_{timestamp}.csv"
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("CHAIN-OF-THOUGHT PROMPT EXPERIMENT RESULTS")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("Chain-of-Thought Prompt Experiment Complete!")
    print("=" * 60)
