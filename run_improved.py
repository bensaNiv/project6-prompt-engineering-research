...
This script executes the improved prompt technique across all 100 test cases,
running each case 2 times to measure consistency. Results are saved to:
- results/improved_results.csv (raw data)
- results/improved_stats.json (aggregated statistics)
"""
...
def main() -> None:
    """Run the improved prompt experiment and generate statistics."""
    print("=" * 60)
    print("Stage 3: Improved Prompt Experiment (Ollama)")
    print("=" * 60)
    ...
    prompt_generator = ImprovedPromptGenerator()
    ...
    # Run improved prompt experiment
    print(f"\n[3/5] Running improved prompt experiment ({total_calls} API calls)...")
    ...
    # Save stats JSON
    stats_path = results_dir / "improved_stats.json"
    with open(stats_path, "w") as f:
        json.dump(stats, f, indent=2)
    print(f"  Saved: {stats_path}")

    # Also copy raw results to expected location
    raw_results_path = results_dir / "improved_results.csv"
    try:
        ...
        alt_path = results_dir / f"improved_results_{timestamp}.csv"
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("IMPROVED PROMPT EXPERIMENT RESULTS")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("Improved Prompt Experiment Complete!")
    print("=" * 60)
